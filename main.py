...
st.write('We used different demographic data for the 50 US states. In some cases where we were not able to fit data for 2020 (as was the case with religions and the political landscape), data were taken for the coming years. More about this in the part of the project devoted to self-criticism.')
...
st.write('We skip the scraping stage, which is fully reflected in the notebook.')
...
st.header('Friedman statistical test to see if the state difference is statistically significant.')
st.code(
"""
...
st.write('We tested the extreme case of Alaska. Its area is so large that it can litter the data. Does it interfere? At the same time, let\'s check our analyze_this function, which displays a summary for a specific relationship between two variables.')
st.code(
"""
analyze_this('area')
# -> Area summary:
...
st.write('We also created a database of variables that show significant patterns, and also recorded them in a dictionary. We used the analysis_list method, which creates a list of calculated variables (r, R, p-value).')
st.code(
"""
insights = {}
...
st.write('The religiosity of a state has nothing to do with its suicide rate. Sun levels associated with less depression, for some reason, do not affect suicide rates in the state. Perhaps this is due to the fact that the US climate is not sufficiently variable. This needs further clarification. High unemployment doesn\'t lead to high suicide rates. For some unknown reason, on the contrary, high unemployment in the state is associated with low suicide rates. The hypothesis of a single value of life (the connection between suicides and murders) was not confirmed.')

st.write('Economic and scientific progress (or rather what we associate with these concepts: GDP, urbanization, political progressivism, religious tolerance) brings a decrease in the level of suicide. The hypothesis requires further reflection.')

st.write('Note that the project already contains a set of ready-made analysis tools (analyze_this, analysis_list, plot_correlation), so in fact, in addition to new data, it requires only minor technical refinement.')
